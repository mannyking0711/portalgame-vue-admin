            ...
            <modal-body v-if="modelShow">
                <form @submit.prevent="save">
                    <grid>
                        ...
                        <form-group
                            :label="translate(translateKey + '.Label.Interest')"
                            name="interest"
                        >
                            <form-text v-model="form.interest"/>
                        </form-group>
                        ...
                        <form-group
                            ...
                            name="type_id"
                        >
                            <form-tree-select :options="discountTypes" v-model="form.type_id"/>
                        </form-group>
...
const translateKey = 'crm.Discount';
...
export default {
    ...
    computed: {
        ...
        ...mapState('DiscountTypesStore', ['discountTypes']),
        permission() {
            return this.currentPage.permission;
        }
    },
    methods: {
        ...mapActions('DiscountStore', ['getDiscounts', 'setDiscount', 'actionDiscount', 'deleteDiscount']),
        ...mapActions('DiscountTypesStore', ['getSelectDiscountTypes']),
        /*
         * Form Create
         * */
        formCreate(item = {}) {
         ...
         * */
        remove(id) {
            this.alert().then(r => this.deleteDiscount(id).then(r => this.getDiscounts()))
        },
        /*
         * Action
         * */
        action(item, type) {
            ...
            this.actionDiscount({id: item.id, type, action}).then(r => this.getDiscounts())
        },
        /*
         * Save
         * */
        save() {
            this.setDiscount(this.form)
            .then(r => {
                if (r) {
                    this.modal(this.modalId);
                    this.getDiscounts();
