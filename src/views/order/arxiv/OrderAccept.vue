                ...
                <grid cols="12">
                    ...
                    <grid-col col="12" v-if="form.game_id">
                        <div>
                            ...
                            <div class="py-4">
                                <grid cols="3">
                                    <div class="border border-gray-200 rounded p-5 relative" v-for="(i, index) in form.customers">
                                    ...
                                    <div class="flex flex-shrink-0 h-60 items-center justify-center border rounded border-gray-200" v-if="form.customers.last.customer_id">
...
export default {
    name: "OrderAccept",
    components: {OrderCustomer, TreeSelect},
    data() {
        return {
            translateKey,
            ...
            normalizer(i) {
                return {
                    id: i.id,
                    label: i.fullname + ( i.phone ? ' (' + i.phone + ')' : '')
                }
            },
            form: formObject()
        }
    ...
    methods: {
        ...
        ...mapActions('CustomerStore', ['getSelectCustomers']),
        ...
        calcPrice(index) {
            const obj = this.form.customers[index];
            let price = 0;
            if (obj) {
        ...
        loadOptions({action, searchQuery, callback}) {
            if (action === ASYNC_SEARCH) {
                this.getSelectCustomers({search: searchQuery, limit: 40})
                .then(r => {
                    callback(null, this.customers)
                })
            }
        },
    },
