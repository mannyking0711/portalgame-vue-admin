                ...
                <form @submit.prevent="save">
                    <grid>
                        ...
                        >
                            ...
                            <!-- form.translates[lang.code].name -->
                        </form-group>
                        ...
                        <form-group
                            :border="false"
                            :label="translate(translateKey + '.Label.File')"
                            name="file"
                        >
                            <form-files :col="3" :photo="photo_url" path="files" img-height="150px" v-model="form.file"/>
                        </form-group>
...
import FormFiles from '../../common/components/form/files/FormFiles.vue';

const modalId = 'createModal';
const translateKey = 'crm.Files';

export default {
  components: { FormFiles },
    name: "FilesIndex",
    data() {
        return {
            translateKey,
    ...
    computed: {
        ...mapState('FilesStore', ['files']),
        permission() {
            return this.currentPage.permission;
        }
    },
    methods: {
        ...mapActions('FilesStore', ['getFiles', 'setFile', 'actionFile', 'deleteFile']),
        /*
         * Form Create
         * */
        formCreate(item = {}) {
         ...
         * */
        remove(id) {
            this.alert().then(r => this.deleteFile(id).then(r => this.getFiles()))
        },
        /*
         * Action
         * */
        action(item, type) {
            ...
            this.actionFile({id: item.id, type, action}).then(r => this.getFiles())
        },
        /*
         * Save
         * */
        save() {
            this.setFile(this.form)
            .then(r => {
                if (r) {
                    this.modal(this.modalId);
                    this.getFiles();
